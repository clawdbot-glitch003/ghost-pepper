...
- **Multi-hop questions** ("do Neville and Peter Pace know each other") need to follow a chain through several files, not score-and-cram one batch.
- **Voice-to-text artifacts** ("He's not a Neville Jun for 10 years" → "He's known Neville for 10 years") need the model to reason past noise — this works when it sees enough surrounding context, not when the right line was edged out by the score-truncation.
- **Citations** ("around line 184 of 2026-01-07/factorial-standup.md") require the model to know exact line numbers, which the cram approach never feeds it.
...
8. **Tool execution UX = collapsed status line + expandable trace.** One status line above the answer ("Reading 2025-01-29/denis-matt.md..."), with a disclosure triangle for the full event log.
...
```json
{
  ...
  "input_schema": {
    "type": "object",
    "properties": {
      "pattern":          {"type": "string", "description": "Regex pattern. Use plain strings for names. Use \\b for word boundaries (e.g., \\bNeville\\b)."},
...
```json
{
  ...
  "input_schema": {
    "type": "object",
    "properties": {
      "path":   {"type": "string",  "description": "Path relative to archive root, e.g., '2025-01-29/denis-matt.md'."},
...
- "He's not a Neville Jun for 10 years" almost certainly means
  "He's known Neville for 10 years."
- "Charlie" addressed in a "Denis <> Matt" meeting is most likely Denis being
...
1. **Status line** — one row above the answer text. Shows the latest tool activity.
   ...
   - While a tool is being dispatched: `"Searching for 'Neville'..."` (grep) / `"Reading 2025-01-29/denis-matt.md (lines 1–200)..."` (read_file) / `"Listing 2026-01-07..."` (list_dir)
...
2. **Expandable trace** — a disclosure triangle next to the status line. Collapsed by default. When expanded, shows the full event log for the current question:
   ...
   ```
   ▼ Trace (8 events)
     [grep]      pattern="Neville", case_insensitive=true → 7 matches in 3 files
     [read_file] 2025-01-29/denis-matt.md offset=1 limit=200 → returned lines 1–200 of 312
     [grep]      pattern="Peter Pace" → 2 matches in 1 file
     [read_file] 2026-01-07/factorial-standup.md offset=170 limit=50 → returned lines 170–219 of 580
     ...
   ```
...
> "Give me a quick timeline of my meetings with Neville."

**Expected behavior:**
- Agent issues `grep("Neville")`.
- Finds matches in `2025-01-29/denis-matt.md`, `2025-05-19/factorial-standup.md`, `2026-01-07/factorial-standup.md`.
...
- Bonus: notes that Neville is *mentioned* but never appears as an attendee.

**Pass criteria:** all three files cited; dates correct; line citations resolve to actual matches.

### Test 2 — Single-meeting summary

> "Tell me about the Denis-Matt meeting."

**Expected behavior:**
- Agent issues `grep("Denis")` or `list_dir("2025-01-29")`.
- Identifies `2025-01-29/denis-matt.md`.
- Reads the file (likely in chunks: frontmatter+summary first, then transcript slices).
- Answer: a multi-section summary (background, fund details, AI investment philosophy, Neville Shaw connection) with line-range citations.
- Bonus: notes that the file title is "Denis <> Matt" but the other speaker is addressed as "Charlie" later in the transcript.

**Pass criteria:** summary covers ≥ 3 substantive topics from the transcript; each section has a `path:start-end` citation; the title vs "Charlie" discrepancy is mentioned.

### Test 3 — Multi-hop with voice-to-text artifact

> "Does Neville know Peter Pace?"

**Expected behavior:**
- Agent issues `grep("Peter Pace")`.
- Finds matches in `2026-01-07/factorial-standup.md`.
- Reads context around line 184: "He's not a Neville Jun for 10 years. Trade ideas. He runs wealth."
- Interprets the voice-to-text glitch: "He's known Neville [Shaw] for ~10 years; they trade ideas."
- Answer: yes, Peter Pace appears to know Neville for ~10 years (per `2026-01-07/factorial-standup.md:184`), with the artifact called out explicitly.
