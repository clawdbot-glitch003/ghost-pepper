...
final class MeetingQAToolsTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        ## Summary
        Discussion about Neville Shaw and the fund.
        """.write(to: rootDir.appendingPathComponent("2025-01-29/denis-matt.md"), atomically: true, encoding: .utf8)
        ...
        **Date:** 2026-01-07
        ...
        ## Notes
        Peter Pace - he's not a Neville Jun for 10 years. Trade ideas.
        """.write(to: rootDir.appendingPathComponent("2026-01-07/factorial-standup.md"), atomically: true, encoding: .utf8)
    }
    ...
    func testGrepFindsMatchAcrossArchive() async throws {
        ...
        let result = try await tools.grep(pattern: "Neville", path: nil, caseInsensitive: true, maxResults: 50)
        XCTAssertTrue(result.contains("2025-01-29/denis-matt.md:"), "Expected file:line match in output: \(result)")
        XCTAssertTrue(result.contains("2026-01-07/factorial-standup.md:"), "Expected second-file match in output: \(result)")
    }

    func testGrepRespectsMaxResults() async throws {
        let tools = MeetingQATools(root: rootDir)
        let result = try await tools.grep(pattern: "Neville", path: nil, caseInsensitive: true, maxResults: 1)
...
```swift
    ...
    func testReadFileReturnsRequestedSliceWithLineNumbers() async throws {
        let tools = MeetingQATools(root: rootDir)
        let result = try await tools.readFile(path: "2025-01-29/denis-matt.md", offset: 1, limit: 200)
        // First content line is "---", line 2 is title.
        XCTAssertTrue(result.hasPrefix("1\t---\n"), "Expected line 1 prefix, got: \(result.prefix(30))")
        XCTAssertTrue(result.contains("2\ttitle: \"Denis <> Matt\""), "Expected line 2: \(result)")
    ...
    func testReadFileWithOffsetSkipsEarlierLines() async throws {
        let tools = MeetingQATools(root: rootDir)
        let result = try await tools.readFile(path: "2025-01-29/denis-matt.md", offset: 5, limit: 1)
...
```swift
    ...
    func testListDirInsideDateFolder() async throws {
        ...
        XCTAssertTrue(result.contains("denis-matt.md"))
        XCTAssertFalse(result.contains("denis-matt.md/"), "File should not have / suffix: \(result)")
    }
...
final class MeetingQASystemPromptTests: XCTestCase {
    ...
    func testPromptHasVoiceToTextGuidance() {
        ...
        XCTAssertTrue(prompt.contains("Neville Jun"), "Should include the canonical artifact example")
    }
...
final class MeetingQAAgentTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        try FileManager.default.createDirectory(at: rootDir, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: rootDir.appendingPathComponent("2025-01-29"), withIntermediateDirectories: true)
        try "Neville Shaw".write(to: rootDir.appendingPathComponent("2025-01-29/denis-matt.md"), atomically: true, encoding: .utf8)
    }
    ...
    func testExecutesToolCallAndContinuesLoop() async throws {
        let provider = MockProvider(scripts: [
            [
                .toolUse(id: "tu_1", name: "grep", input: ["pattern": "Neville", "case_insensitive": true, "max_results": 50]),
                .stop(reason: .toolUse, usage: ProviderUsage(inputTokens: 50, outputTokens: 10, cacheReadTokens: 0, cacheWriteTokens: 0)),
            ],
            [
                .textDelta("Found in 2025-01-29/denis-matt.md."),
        ...
        var text = ""
        for try await event in agent.ask("Where is Neville?") {
            switch event {
        ...
        XCTAssertEqual(text, "Found in 2025-01-29/denis-matt.md.")
...
**Pass criteria:**
- Status line cycles through `Searching: pattern="Neville"...` → `Reading <file>...` (one or more times) → `Thinking...`.
- Final answer chronologically lists three files: `2025-01-29/denis-matt.md`, `2025-05-19/factorial-standup.md`, `2026-01-07/factorial-standup.md`.
- Each entry has a `path:line` citation.
- Answer mentions Neville is mentioned but never an attendee.
...
**Pass criteria:**
- Trace shows at least one `read_file` call into `2025-01-29/denis-matt.md`.
- Final answer covers ≥ 3 substantive topics with `path:start-end` citations.
- Bonus pass: answer notes the title is "Denis <> Matt" but the speaker is later addressed as "Charlie".
...
**Pass criteria:**
- Trace shows `grep("Peter Pace")` and a `read_file` into `2026-01-07/factorial-standup.md` covering line ~184.
- Final answer correctly interprets "He's not a Neville Jun for 10 years" as "He's known Neville for ~10 years."
